...
use EDCARE_CORE_ELEMENTOR\Templates\TP_Api as TemplatesTP_Api;
...
class TP_Api extends Source_Base {
	...
	private static function request_library_data( $force_update = false ) {
		$response	=	wp_remote_get(EDCARE_CORE_API_URL .'data.json',['timeout'=>60]);
	...
	public static function request_template_data( $template_id ) {
		...
		$template_url = sprintf(EDCARE_CORE_API_URL . "templates/%s.json", $template_id);
