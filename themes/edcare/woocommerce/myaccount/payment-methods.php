						...
						<td class="woocommerce-PaymentMethod woocommerce-PaymentMethod--<?php echo esc_attr( $column_id ); ?> payment-method-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							if ( has_action( 'woocommerce_account_payment_methods_column_' . $column_id ) ) {
								...
								if ( ! empty( $method['method']['last4'] ) ) {
									...
									echo sprintf( esc_html__( '%1$s ending in %2$s', 'edcare' ), esc_html( wc_get_credit_card_type_label( $method['method']['brand'] ) ), esc_html( $method['method']['last4'] ) );
								} else {
...
<?php else : ?>
	...
	<?php wc_print_notice( esc_html__( 'No saved methods found.', 'edcare' ), 'notice' ); ?>
...
<?php if ( WC()->payment_gateways->get_available_payment_gateways() ) : ?>
	<a class="button" href="<?php echo esc_url( wc_get_endpoint_url( 'add-payment-method' ) ); ?>"><?php esc_html_e( 'Add payment method', 'edcare' ); ?></a>
<?php endif; ?>
