...
$enroll_btn = '<div class="tutor-course-list-btn">' . apply_filters( 'tutor_course_restrict_new_entry', '<a href="' . get_the_permalink() . '" class="tutor-btn tutor-btn-outline-primary tutor-btn-md tutor-btn-block ' . $required_loggedin_class . '">' . __( 'Enroll Course', 'edcare' ) . '</a>' ) . '</div>';
...
if ( tutor_utils()->is_course_purchasable() ) {
	...
	if ( false === $product ) {
		echo edcare_kses($free_html); //phpcs:ignore --contain safe data
		...
		echo '<div class="tutor-course-booking-availability"> ' .
	...
	if ( $product && $maximum_students === $total_enrolled && 0 !== $maximum_students ) {
		...
		$restrict   = '<div class="list-item-booking booking-full tutor-d-flex tutor-align-center"><div class="booking-progress tutor-d-flex"><span class="tutor-mr-8 tutor-color-warning tutor-icon-circle-info"></span></div><div class="tutor-fs-7 tutor-fw-medium tutor-color-black">' . __( 'Fully Booked', 'edcare' ) . '</div></div>';
		
		echo edcare_kses($restrict); //phpcs:ignore --contain safe data
	}

	if ( $product && 0 === $maximum_students ) {
		$price_html = '<div class=" ' . $utility_classes . ' "><div class="list-item-price tutor-d-flex tutor-align-center"> <span class="price tutor-fs-6 tutor-fw-bold tutor-color-black">' . $wc_price_html . ' </span></div>';
		$cart_html  = '<div class="list-item-button"> ' . apply_filters( 'tutor_course_restrict_new_entry', $enroll_btn ) . ' </div>';
		
		echo edcare_kses($cart_html); //phpcs:ignore
	}
} else {
	echo edcare_kses($free_html); //phpcs:ignore --contain safe data
}
