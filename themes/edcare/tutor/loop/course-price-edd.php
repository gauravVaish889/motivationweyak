...
<?php
	...
	$enroll_btn = '<div class="list-item-button">' . apply_filters( 'tutor_course_restrict_new_entry', '<a href="' . get_the_permalink() . '" class="tutor-btn tutor-btn-outline-primary tutor-btn-md tutor-btn-block">' . __( 'Enroll Course', 'edcare' ) . '</a>' ) . '</div>';
...
if ( tutor_utils()->is_course_purchasable() ) {
	...
	if ( $maximum_students == $total_enrolled && $maximum_students != 0 ) {
		...
		$restrict = '<div class="list-item-booking booking-full tutor-d-flex tutor-align-center">
                                ...
                                <div class="tutor-fs-7 tutor-fw-medium tutor-color-black">' .
							__( 'Fully Booked', 'edcare' ) . '
                                </div>
                            </div></div>';
		echo edcare_kses($price_html);//phpcs:ignore --trusted markups
		echo edcare_kses($restrict);//phpcs:ignore --trusted markups
	}
