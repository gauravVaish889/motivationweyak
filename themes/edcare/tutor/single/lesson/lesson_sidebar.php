...
<div class="tutor-course-single-sidebar-title tutor-d-flex tutor-justify-between">
	<span class="tutor-fs-6 tutor-fw-medium tutor-color-secondary"><?php esc_html_e( 'Course Content', 'edcare' ); ?></span>
...
if ( $topics->have_posts() ) {
	...
	while ( $topics->have_posts() ) {
		...
		<div class="tutor-course-topic tutor-course-topic-<?php echo esc_attr( $topic_id ); ?>">
			<div class="tutor-accordion-item-header<?php echo esc_attr($is_topic_active ? ' is-active' : ''); ?>" tutor-course-single-topic-toggler>
			...
			<div class="tutor-accordion-item-body <?php echo esc_attr($is_topic_active ? '' : 'tutor-display-none'); ?>">
				<?php
				...
				while ( $lessons->have_posts() ) {
					...
					if ( 'tutor_quiz' === $post->post_type ) {
						...
						<div class="tutor-course-topic-item tutor-course-topic-item-quiz<?php echo ( get_the_ID() == $current_post->ID ) ? ' is-active' : ''; ?>" data-quiz-id="<?php echo esc_attr( $quiz->ID ); ?>">
							<a href="<?php echo esc_attr($show_permalink ? esc_url( get_permalink( $quiz->ID ) ) : '#'); ?>" data-quiz-id="<?php echo esc_attr( $quiz->ID ); ?>">
					...
					<?php } elseif ( 'tutor_assignments' === $post->post_type ) { ?>
						<div class="tutor-course-topic-item tutor-course-topic-item-assignment<?php echo esc_attr( get_the_ID() == $current_post->ID ? ' is-active' : '' ); ?>">
							<a href="<?php echo esc_attr($show_permalink ? esc_url( get_permalink( $post->ID ) ) : '#'); ?>" data-assignment-id="<?php echo esc_attr( $post->ID ); ?>">
								<div class="tutor-d-flex tutor-mr-32">
					...
					<?php } elseif ( 'tutor_zoom_meeting' === $post->post_type ) { ?>
						<div class="tutor-course-topic-item tutor-course-topic-item-zoom<?php echo esc_attr( ( get_the_ID() == $current_post->ID ) ? ' is-active' : '' ); ?>">
							<a href="<?php echo esc_attr($show_permalink ? esc_url( get_permalink( $post->ID ) ) : '#'); ?>">
								<div class="tutor-d-flex tutor-mr-32">
					...
					<?php } else { ?>
						...
						<div class="tutor-course-topic-item tutor-course-topic-item-lesson<?php echo esc_attr( get_the_ID() == $current_post->ID ? ' is-active' : '' ); ?>">
							<a href="<?php echo esc_attr($show_permalink ? esc_url( get_the_permalink() ) : '#'); ?>" data-lesson-id="<?php the_ID(); ?>">
								<div class="tutor-d-flex tutor-mr-32">
									<?php
