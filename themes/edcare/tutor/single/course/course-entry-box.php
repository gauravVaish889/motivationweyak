...
$auth_url                = $is_tutor_login_disabled ? wp_login_url( tutor_utils()->get_current_url()) : '';
$default_meta            = array(
	array(
		'icon_class' => 'tutor-icon-mortarboard',
		'label'      => __( 'Total Enrolled', 'edcare' ),
		'value'      => tutor_utils()->get_option( 'enable_course_total_enrolled' ) ? tutor_utils()->count_enrolled_users_by_course() . ' ' . __( 'Total Enrolled', 'edcare' ) : null,
	),
	array(
		'icon_class' => 'tutor-icon-clock-line',
		'label'      => __( 'Duration', 'edcare' ),
		'value'      => get_tutor_option( 'enable_course_duration' ) ? ( get_tutor_course_duration_context() ? get_tutor_course_duration_context() . ' ' . __( 'Duration', 'edcare' ) : false ) : null,
	),
	array(
		'icon_class' => 'tutor-icon-refresh-o',
		'label'      => __( 'Last Updated', 'edcare' ),
		'value'      => get_tutor_option( 'enable_course_update_date' ) ? get_the_modified_date( get_option( 'date_format' ) ) . ' ' . __( 'Last Updated', 'edcare' ) : null,
	),
);
		...
		if ( $is_enrolled || $is_privileged_user ) {
			...
			<?php if ( tutor_utils()->get_option( 'enable_course_progress_bar', true, true ) && is_array( $course_progress ) && count( $course_progress ) ) : ?>
				<div class="tutor-course-progress-wrapper tutor-mb-32">
					<h3 class="edcare-course-single-sidebar-title">
						<?php esc_html_e( 'Course Progress', 'edcare' ); ?>
					</h3>
			...
			if ( $lesson_url ) {
				...
				if ( $retake_course && ( CourseModel::MODE_FLEXIBLE === $completion_mode || $is_completed_course ) ) {
					?>
					<button type="button" 
							class="edcare-course-btn edcare-course-btn-outline w-100 start-continue-retake-button tutor-course-retake-button" 
							href="<?php echo esc_url( $lesson_url ); ?>"
							data-course_id="<?php echo esc_attr( get_the_ID() ); ?>">
						<?php esc_html_e( 'Retake This Course', 'edcare' ); ?>
					</button>
					<?php
				}
				...
				if ( ! $is_completed_course ) {
					if ( 0 === (int) $completed_percent ) {
						$link_text = __( 'Start Learning', 'edcare' );
					}
					if ( $completed_percent > 0 && $completed_percent < 100 ) {
						$link_text = __( 'Continue Learning', 'edcare' );
					}
					...
					if ( 100 === (int) $completed_percent && false === CourseModel::can_complete_course( $course_id, $user_id ) ) {
						...
						$link_text  = __( 'Review Progress', 'edcare' );
					}
				}
			...
			if ( ! $is_completed_course ) {
				...
				<form method="post" class="mt-10">
					...
					<button type="submit" class="edcare-course-btn edcare-course-btn-outline w-100" name="complete_course_btn" value="complete_course">
						<?php esc_html_e( 'Complete Course', 'edcare' ); ?>
					</button>
				</form>
				<?php
			...
			?>
				...
				if ( '' !== $post_date ) :
					?>
					<div class="tutor-fs-7 tutor-color-muted tutor-mt-20 tutor-d-flex tutor-justify-center">
						<span class="tutor-fs-6 tutor-color-success tutor-icon-purchase-mark tutor-mr-8"></span>
						<span class="tutor-enrolled-info-text">
						<?php esc_html_e( 'You enrolled in this course on', 'edcare' ); ?>
			...
			?>
				<a href="<?php echo esc_url( $first_lesson_url ); ?>" class="tutor-btn tutor-btn-primary tutor-btn-lg tutor-btn-block">
					<?php esc_html_e( 'Start Learning', 'edcare' ); ?>
				</a>
			<?php
			...
			if ( tutor_utils()->is_course_fully_booked( null ) ) {
				...
				?>
					<div class="tutor-alert tutor-warning tutor-mt-28">
						<div class="tutor-alert-text">
							...
							<span>
								<?php esc_html_e( 'This course is full right now. We limit the number of students to create an optimized and productive group dynamic.', 'edcare' ); ?>
							</span>
						</div>
					</div>
				<?php
				...
				?>
					<div class="tp-course-details2-widget-price">
						<span class="lms-free">
							<?php esc_html_e( 'Free', 'edcare' ); ?>
						</span>
					</div>

					<div class="tutor-course-single-btn-group tp-course-details-2-widget-btn <?php echo is_user_logged_in() ? '' : 'tutor-course-entry-box-login'; ?>" data-login_url="<?php echo esc_url( $login_url ); ?>">
						<form class="tutor-enrol-course-form" method="post">
							<?php wp_nonce_field( tutor()->nonce_action, tutor()->nonce ); ?>
							<input type="hidden" name="tutor_course_id" value="<?php echo esc_attr( get_the_ID() ); ?>">
							<input type="hidden" name="tutor_course_action" value="_tutor_course_enroll_now">
							<button type="submit" class="edcare-course-btn w-100 tutor-enroll-course-button tutor-static-loader">
								<?php esc_html_e( 'Enroll Now', 'edcare' ); ?>
							</button>
						</form>
					</div>

					<div class="tutor-fs-7 tutor-color-muted tutor-mt-20 tutor-text-center">
						<?php esc_html_e( 'Free access this course', 'edcare' ); ?>
					</div>
				<?php
