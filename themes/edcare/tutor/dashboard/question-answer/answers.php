...
?>
<h2><?php esc_html_e('Answer', 'edcare'); ?></h2>
		...
		<div class="tutor_original_question">
			<div class="tutor-question-wrap">
				<div class="question-top-meta">
					...
					<p class="review-meta">
						<a
							href="<?php echo esc_url($profile_url); ?>"><?php echo esc_html($question->display_name); ?></a>
						<span class="tutor-text-mute">
							<?php
							/* translators: %s: time */
							echo esc_html(wp_sprintf(__('%s ago', 'edcare'), human_time_diff(strtotime($question->comment_date_gmt))));
							?>
						</span>
					</p>
				</div>
				...
				<div class="tutor_question_area">
					<p><strong><?php echo esc_html($question->question_title); ?> </strong></p>
		...
		<div class="tutor_admin_answers_list_wrap">
			...
			if (is_array($answers) && count($answers)) {
				foreach ($answers as $answer) {
					...
					?>
					<div
						class="tutor_individual_answer <?php echo esc_attr(($question->user_id == $answer->user_id) ? 'tutor-bg-white' : 'tutor-bg-light'); ?>">
						<div class="tutor-question-wrap">
							<div class="question-top-meta">
								<div class="tutor-question-avater">
								...
								<p class="review-meta">
									<a
										href="<?php echo esc_url($answer_profile); ?>"><?php echo esc_html($answer->display_name); ?></a>
									<span class="tutor-text-mute">
										<?php echo esc_html(wp_sprintf(__('%s ago', 'edcare'), human_time_diff(strtotime($answer->comment_date_gmt)))); ?>
									</span>
								</p>
							</div>
			...
			<div class="tutor-add-question-wrap">
				<form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" id="tutor_admin_answer_form"
					method="post">
					...
					<div class="tutor-form-group">
						<button type="submit" class="tutor-button tutor-button-primary"
							name="tutor_question_search_btn"><?php esc_html_e('Reply', 'edcare'); ?> </button>
					</div>
				</form>
			</div>
		</div>
