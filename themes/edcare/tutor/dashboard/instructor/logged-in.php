...
if ( 'try_again' === $instructor_status ) {
	...
	<div class="tutor-alert tutor-warning tutor-d-flex tutor-align-center">
		...
		<span><?php esc_html_e( 'You have been rejected from being an instructor.', 'edcare' ); ?></span>
	</div>
	<?php
		...
		<div class="tutor-instructor-application-process tutor-pt-48 tutor-pb-48">
			<div class="tutor-app-process-alert">
				<div style="border:1px solid var(--tutor-color-primary);" class="tutor-primary tutor-py-12 tutor-px-20 tutor-radius-6">
					...
					if ( 'pending' == $instructor_status ) {
						esc_html_e( 'Your application will be reviewed and the results will be sent to you by email.', 'edcare' );
					} elseif ( 'approved' == $instructor_status ) {
						esc_html_e( 'Your application has been accepted. Further necessary details have been sent to your registered email account.', 'edcare' );
					} elseif ( 'blocked' == $instructor_status ) {
						esc_html_e( 'You have been blocked from being an instructor.', 'edcare' );
					}
					?>
					</span>
					</div>
			...
			<div class="tutor-app-process-image tutor-m-auto tutor-pt-32 tutor-pb-44 tutor-d-flex tutor-justify-center tutor-align-center">
				<span class="tutor-app-process-img">
					...
					alt="<?php esc_attr_e( 'Instructor Application Received', 'edcare' ); ?>"
					/>
				</span>
			</div>
			<div class="tutor-instructor-application-body">
				<div class="tutor-ins-app-title tutor-m-auto tutor-text-center">
					...
					<?php
					if ( 'pending' == $instructor_status ) {
						esc_html_e( 'Thank you for registering as an instructor! ', 'edcare' );
					} elseif ( 'approved' == $instructor_status ) {
						esc_html_e( 'Congratulations! You are now registered as an instructor.', 'edcare' );
					} elseif ( 'blocked' == $instructor_status ) {
						esc_html_e( 'Unfortunately, your instructor status has been removed.', 'edcare' );
					}
					?>
					</span>
				</div>
				<div class="tutor-ins-app-subtitle tutor-m-auto tutor-text-center tutor-pt-24 tutor-pb-48">
					...
					<?php
					if ( 'pending' == $instructor_status ) {
						esc_html_e( 'We\'ve received your application, and we will review it soon. Please hang tight!', 'edcare' );
					} elseif ( 'approved' == $instructor_status ) {
						esc_html_e( 'Start building your first course today and let your eLearning journey begin.', 'edcare' );
					} elseif ( 'blocked' == $instructor_status ) {
						esc_html_e( 'Please contact the site administrator for further information.', 'edcare' );
					}
					?>
					</span>
				</div>
				<div class="tutor-instructor-apply-button tutor-text-center">
					<a style="text-decoration:none;" class="tutor-bg-primary tutor-color-white tutor-py-16 tutor-px-32 tutor-radius-6" href="<?php echo esc_url( tutor_utils()->tutor_dashboard_url() ); ?>">
						<?php esc_html_e( 'Go to Dashboard', 'edcare' ); ?>
					</a>
				</div>
			</div>
		</div>
