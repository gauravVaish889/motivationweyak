...
$setting_menus = array(
	'profile'        => array(
		...
		'title' => __( 'Profile', 'edcare' ),
		'role'  => false,
	),
	'reset_password' => array(
		'url'   => esc_url( $reset_password ),
		'title' => __( 'Password', 'edcare' ),
		'role'  => false,
	),
	'withdrawal'     => array(
		'url'   => esc_url( $withdraw ),
		'title' => __( 'Withdraw', 'edcare' ),
		'role'  => 'instructor',
	),
	'social-profile' => array(
		'url'   => esc_url( $social ),
		'title' => __( 'Social Profile', 'edcare' ),
		'role'  => false,
	),
);
...
<ul class="tutor-nav border-0" tutor-priority-nav>
	<?php foreach ( $setting_menus as $menu_key => $menu ) : ?>
		...
		<?php if ( $valid ) : ?>
			<li class="tutor-nav-item">
				<a class="tutor-nav-link<?php echo esc_attr($active_setting_nav == $menu_key ? ' is-active' : ''); ?>" href="<?php echo esc_url( $menu['url'] ); ?>"><?php echo esc_html( $menu['title'] ); ?></a>
			</li>
		<?php endif; ?>
	<?php endforeach; ?>
