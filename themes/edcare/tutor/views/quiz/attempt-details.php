...
if ( ! $attempt_id || ! $attempt_data || $user_id != $attempt_data->user_id ) {
	tutor_utils()->tutor_empty_state( __( 'Attempt not found or access permission denied', 'edcare' ) );
	return;
}
			...
			switch ( $type ) {
				...
				case 'text':
					...
					if ( isset( $ans->answer_title ) && ! isset( $ans->answer_two_gap_match ) ) {
						...
						echo edcare_kses($ans_string); //phpcs:ignore -- contain safe data
					}
					break;
		...
		<thead>
			<tr>
				<?php foreach ( $table_1_columns as $key => $column ) : ?>
					<th><?php echo edcare_kses($column); //phpcs:ignore --contain safe data ?></th>
				<?php endforeach; ?>
			</tr>
		</thead>
		...
		<tbody>
			<tr>
				<?php foreach ( $table_1_columns as $key => $column ) : ?>
					...
					<?php
					if ( 'date' === $key ) :
						esc_html_e( 'min-width:180px', 'edcare' );
					endif;
					?>
					">
						<?php if ( 'user' == $key ) : ?>
						...
						<?php elseif ( 'result' == $key ) : ?>
							...
							if ( $has_pending ) {
								echo '<span class="tutor-badge-label label-warning">' . esc_html__( 'Pending', 'edcare' ) . '</span>';
							} elseif ( $earned_percentage >= $pass_mark_percent ) {
								echo '<span class="tutor-badge-label label-success">' . esc_html__( 'Pass', 'edcare' ) . '</span>';
							} else {
								echo '<span class="tutor-badge-label label-danger">' . esc_html__( 'Fail', 'edcare' ) . '</span>';
							}
							?>
						<?php endif; ?>
					</td>
...
if ( is_array( $answers ) && count( $answers ) ) {
	echo 'course-single-previous-attempts' != $context ? '<div class="tutor-fs-6 tutor-fw-medium tutor-color-black tutor-mt-24">' . esc_html__( 'Quiz Overview', 'edcare' ) . '</div>' : '';
	?>
										...
										case 'result':
											?>
												...
												switch ( $answer_status ) {
													case 'correct':
														echo '<span class="tutor-badge-label label-success">' . esc_html__( 'Correct', 'edcare' ) . '</span>';
														break;
													...
													case 'wrong':
														echo '<span class="tutor-badge-label label-danger">' . esc_html__( 'Incorrect', 'edcare' ) . '</span>';
														break;
												}
												?>
												</td>
										...
										case 'manual_review':
											?>
												<td class="tutor-text-center tutor-nowrap-ellipsis">
													<a href="javascript:;" data-back-url="<?php echo esc_url( $back_url ); ?>" data-attempt-id="<?php echo esc_attr( $attempt_id ); ?>" data-attempt-answer-id="<?php echo esc_attr( $answer->attempt_answer_id ); ?>" data-mark-as="correct" data-context="<?php echo esc_attr( $context ); ?>" title="<?php esc_attr_e( 'Mark as correct', 'edcare' ); ?>" class="quiz-manual-review-action tutor-mr-12 tutor-icon-rounded tutor-color-success">
														<i class="tutor-icon-mark"></i>
													</a>

													<a href="javascript:;" data-back-url="<?php echo esc_url( $back_url ); ?>" data-attempt-id="<?php echo esc_attr( $attempt_id ); ?>" data-attempt-answer-id="<?php echo esc_attr( $answer->attempt_answer_id ); ?>" data-mark-as="incorrect" data-context="<?php echo esc_attr( $context ); ?>" title="<?php esc_attr_e( 'Mark as In correct', 'edcare' ); ?>" class="quiz-manual-review-action tutor-icon-rounded tutor-color-danger">
