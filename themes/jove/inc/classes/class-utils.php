...
class Utils {
    ...
    public static function get_api_data( $title, $type ) {
        ...
        $params = [
            ...
            'category_filter'   => ( $type === 'experimentResponse' ) ? ["journal"] : ["jove_core"]
        ];
         ...
         */
        curl_setopt( $ch, CURLOPT_URL, 'https://api.jove.com/api/free/search/search_ai' );
        ...
        if ( !empty( $decoded_response['content']['result'] ) ) {
            foreach ($decoded_response['content']['result'] as $key => $value) {
                $output['data'][$type][$key]['joveArticleId'] = $value['id'];
                $output['data'][$type][$key]['joveTitle'] = $value['title'];
                ...
                $output['data'][$type][$key]['video'] = "https://app.jove.com/v/" . $value['id'];
    ...
    public static function fetch_visualize_data( $id ) {
        //$api_url = 'https://visualize-api.jove.com/jove/expriment-concept/' . $id; // production
        $api_url = 'https://visualize-api.dev.test.jove.com/jove/expriment-concept/' . $id; // dev
